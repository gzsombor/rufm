...
pub fn draw<B: Backend> // <Backend: tui::backend::Backend>
    ...
    terminal.draw(|mut f| {
        ...
        let mut info_pgraph = Paragraph::new(info_display.iter())
            .block(custom_block.title(info.get_title()).border_style(info.border_style))
            .style(Style::default().fg(Color::White))
            .alignment(Alignment::Left)
            .wrap(true);
        ...
        match selected {
            ...
            Selectable::Info => {
                // add colored border
                info_pgraph = info_pgraph.block(custom_block.title(info.get_title()).border_style(custom_border_style_selected));
            },
